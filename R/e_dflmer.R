e_dflmer<-function(explaining=0){
  ...
  if(explaining==0) {
    ...
    cat("   ", '\n')
    cat("\033[1;32m## nlme: Optimization ---------- \033[0m", '\n')
    ...
    cat("  IOM <- lme(salary~experience + sector, random=~1|firma, data=df, control=lmeControl(opt='optim', optCtrl=list(method='L-BFGS-B')) ) ", '\n')
    cat("   ", '\n')
    cat("\033[1;32m## lmer: Optimization ---------- \033[0m", '\n')
    cat("  IOM <- lmer(salary ~ 1  + (1 | firma), data = df, REML = TRUE, control = lmerControl(optimizer ='Nelder_Mead') ) ", '\n')
    cat("  IOM <- lmer(salary ~ 1  + (1 | firma), data = df, REML = TRUE, control = lmerControl(optimizer ='optimx', optCtrl=list(method='L-BFGS-B')) )  ", '\n')
    cat("   ", '\n')
    cat("  library(dplyr); library(dflmer); library(lme4); library(lmerTest); library(aj412s); library(optimx)", '\n')
    cat("  ", '\n')
    cat("\033[1;32m## ICC function ----------- \033[0m", '\n')
    cat("   ", '\n')
    ...
    cat("     IOM <- lmer(salary ~ 1  + (1 | firma), data = df, REML = TRUE ) ", '\n')
    cat("     ranova(IOM)", '\n')
    cat("   ", '\n')
    ...
    cat("\033[1;33m## gc:Level-1변수, gpm:Level-2변수, gc는 기업 평균에서 벗어난 개인의 experience\033[0m", '\n')
    cat("     df <- df %>% group_by(firma) %>% mutate(experience_gpm = mean(experience), experience_gc = experience - experience_gpm) %>% ungroup()", '\n')
    cat("   ", '\n')
    cat("     df <- df %>% dplyr::select(firma, sector, experience, experience_gpm, experience_gc, salary)", '\n')
    cat("  ", '\n')
    cat("\033[1;31m     ##Context effect Model \033[0m", '\n')
    cat("     context.model <- lmer(salary ~ experience_gc + experience_gpm + (1 | firma), data = df, REML = TRUE)", '\n')
    cat("  ", '\n')
