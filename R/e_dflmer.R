...
cat("  ", '\n')
cat("  ## Random Coefficients Modell (RCM) M3 ------", '\n')
...
cat("  ", '\n')
cat("  ## General intercept-as-outcome model M4 -----", '\n')
cat("     intercept.as.outcome.model <- lmer(salary ~ experience + sector + (experience | firma), data = df, REML = TRUE) ", '\n')
cat("     df$intercept.as.outcome.preds <- predict(intercept.as.outcome.model)", '\n')
cat("  ", '\n')
cat("  ## Slope-as-outcome model -----", '\n')
