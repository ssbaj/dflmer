...
cat("     slope.as.outcome.model <- lmer(salary ~ experience + sector + (experience:sector) + (experience | firma), data = df, REML = TRUE)", '\n')
...
cat("  ﻿   model1b <- glmer(y ~ severe +  Rx + time + Rx*time + (1 | id), data=depress, family=binomial)", '\n')
cat("  ", '\n')
